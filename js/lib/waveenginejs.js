...
var WaveEngineJS = {
         ...
         * @return {Array(3)}
         */
        PlusV3:function (vector1, vector2) {
        ...
        SphereCamera:function (width, height) {
            ...
            this.OnRotateCamera = function (eventType, position) {
                ...
                if ((eventType == "mousemove" || eventType == "MSPointerMove" || eventType == "touchmove") && that.isDraggin) {
                    if (that.isDragThresholdEnabled) {
                        var diff = { x:that.downPosition2D.x - position.x, y:that.downPosition2D.y - position.y };
                        that.hasDragged = !(Math.abs(diff.x) < that.dragThreshold || Math.abs(diff.y) < that.dragThreshold);
                    }
                    else {
                        that.hasDragged = true;
                    }
        ...
        Model:function (id) {
            ...
            this.AddColor = function (color) {
                var position = that.colors.length;
                that.colors[position] = color;
            };
            ...
            this.RefreshWorld = function () {
                ...
                var scaleMatrix = new WaveEngineJS.CreateScale(that.scale[0], that.scale[1], that.scale[2]);
                var translationMatrix = new WaveEngineJS.CreateTranslation(that.position);
        ...
        GroupModel:function (id) {
            ...
            this.RefreshWorld = function () {
                ...
                for (var i = 0; i < l; i++) {
                    ...
                    models[i].world = WaveEngineJS.Multiply(models[i].world, that.world);
                }
                that.refreshedBoudingBox = false;
            };
        ...
        Scene:function (canvas) {
            ...
            this.Render = function (type) {
                ...
                for (var i = 0; i < sortModels.length; i++) {
                    ...
                    var worldviewproj = WaveEngineJS.Multiply(model.world, that.camera.viewproj);
                    var vertices = new Array();
                ...
                for (var i = indexes.length / 3; i--;) {
                    ...
                    if (!model.backfaceCulling || tita < 0) {
                        if (hasIllumination) {
                            ...
                            var r = sat * modelColorR + that.ambientColor[0];
                            var g = sat * modelColorG + that.ambientColor[1];
                            var b = sat * modelColorB + that.ambientColor[2];
                            r = r | r;
                            g = g | g;
                            b = b | b;
